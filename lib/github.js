  ...
  if ( OpenIssues > 0 ){
    ...
    if ( gitHubParagraph.labels.length !== 0 ){
      for ( idxLabel in gitHubParagraph.labels ) {
        ...
        await generateBadge(gitHubParagraph.labels[idxLabel]);
        gitHubParagraph.textParagraph += `${gitHubParagraph.badgeWithLink(gitHubParagraph.labels[idxLabel].title, '')}`;
        gitHubParagraph.returnCarrige();
      }  
    } 
  }
...
const GitHub = class GitHub extends documentStatus {
  ...
  /**
   * Get url fron opeb issues
   * 
   * @memberof Github#urlOpenIssueLabel
   * @param {String} - issue label
   * @return {String} - url to issue label
   */  
  urlOpenIssueLabel(issueLabel) {
    let responseUrl = null;
    const rootUrl = require('./../package.json').repository.url.replace('git+', '').replace('.git', '');
    responseUrl = `${rootUrl}/issues?q=is%3Aissue+is%3Aopen+label%3A${encodeURI(issueLabel)}`;
    return responseUrl;
  }
   ...
   * Get url fron opeb issues
   * 
   * @memberof Github#badgeWithLink
   * @param {String, String} - title badge, url badge
   * @return {String} - text paragraph
   */  
  badgeWithLink(title){
    return this.imagelink(title, this.urlOpenIssueLabel(title));
  } 
}
